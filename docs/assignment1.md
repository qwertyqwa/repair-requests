...
### 2.1.1. Входные данные (работа мастера)
- **Комментарий** (текст, необязательно; сохраняется в журнале комментариев)
- **Комплектующие** (наименование + количество; список позиций)
...
- История смены статусов по заявке.
- Уведомления о смене статуса (для оператора/администратора).
...
6. При выполнении заявки мастером:
   ...
   3) мастер добавляет комментарии и фиксирует комплектующие отдельными записями;
   4) система сохраняет изменения; при переводе в «В ремонте» фиксирует время начала, при переводе в «Готова» фиксирует время завершения;
   5) система записывает изменение статуса в историю и создаёт уведомления оператору/администратору.
...
- `Уведомления` — список уведомлений о смене статуса.
